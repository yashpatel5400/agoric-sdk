...
/** @typedef {ZoeService} ZoeServiceWFeePurseApplied FIXME: Remove */
...
export function makeWallet({
  ...
  const wallet = Far('wallet', {
    ...
    getUINotifier,
    getZoe() {
      // Do *not* return paidZoe, since it is sensitive.
      return zoe;
    },
    getBoard() {
      return board;
  ...
  const makePaidZoe = async feePurse => {
    /** @type {*} FIXME: Remove */
    const untypedRawZoe = zoe;
    if (!feePurse) {
      return untypedRawZoe;
    }
    return E(untypedRawZoe).applyFeePurse(feePurse);
  };
  ...
  const initialize = async () => {
    ...
    const addZoeFeePurse = async ({ proposedName, issuerName }) => {
      /** @type {*} FIXME: Remove */
      const untypedZoe = zoe;
      const feeIssuer = await E(untypedZoe)
        .getFeeIssuer()
        // TODO: Remove this when the fee system is fully implemented.
        .catch(() => undefined);

      if (!feeIssuer) {
        // Not implemented in Zoe.
        return undefined;
      }

      // Install the fee purse.
      await addIssuer(issuerName, feeIssuer);
      const purse = await E(untypedZoe).getFeePurse();
    ...
    paidZoe = await makePaidZoe(zoeFeePurse);
    ...
    const inviteIssuerP = E(paidZoe).getInvitationIssuer();
