...
export function makeVirtualObjectManager(
  ...
  */
   ...
   * makeWeakStore() instance. We currently never remove anything from the set,
   * but eventually we'll use refcounts within virtual data to figure out when
   * the vref becomes unreachable, allowing the vat to send a dropImport into
   * the kernel and release the object.
   *
   */
   ...
   * or virtual object as a key into a makeWeakStore() instance or an instance
   * of VirtualObjectAwareWeakMap or VirtualObjectAwareWeakSet.  We remove it
  ...
  function removeRecognizableVref(value, recognizer) {
    const vref = getSlotForVal(value);
    if (vref) {
      const { type, allocatedByVat, virtual } = parseVatSlot(vref);
      if (type === 'object' && (!allocatedByVat || virtual)) {
    ...
    const result = harden({
      ...
      delete(key) {
        ...
        if (vkey) {
          ...
          removeRecognizableVref(key, deleter);
        } else {
          assertKeyExists(key);
  ...
  class VirtualObjectAwareWeakMap {
    constructor() {
      ...
      const virtualObjectMap = new Map();
      virtualObjectMaps.set(this, virtualObjectMap);
      droppedCollectionRegistry.register(this, virtualObjectMap, this);
    }
    ...
    delete(key) {
      ...
      if (vkey) {
        const vmap = virtualObjectMaps.get(this);
        if (vmap.has(vkey)) {
          removeRecognizableVref(key, vmap);
  ...
  class VirtualObjectAwareWeakSet {
    constructor() {
      ...
      const virtualObjectSet = new Set();
      virtualObjectSets.set(this, virtualObjectSet);
      droppedCollectionRegistry.register(this, virtualObjectSet, this);
    }
    ...
    delete(value) {
      ...
      if (vkey) {
        const vset = virtualObjectSets.get(this);
        if (vset.has(vkey)) {
          removeRecognizableVref(value, vset);
